...
class Knight(Thread):
    ...
    def run(self):
        ...
        while self.enemies > 0:
            ...
            if self.enemies < 0:
                self.enemies = 0
            print(f'{self.name} сражается {self.days} суток,  осталось {self.enemies} воинов врага.')
        print(f'{self.name} одержал победу спустя {self.days} дней !')

if __name__ == '__main__':


    first_knight = Knight('Илья Муромец',15)
    second_knight = Knight('Алёша Попович',20)
    first_knight.start()
    second_knight.start()
    first_knight.join()
    second_knight.join()
